...
-- snips are not separated in paragraphs
-- but the sentences are part of the snip and the snip part of the book
...
mkSentenceTriple2 lang snipid sent
    ...
    where
        ...
        toktrips =  concatMap (mkTokenTriple2 lang sentSigl)
                                $ (stoks sent):: [Triple]
        sentenceForm = mkTripleLang3 lang (unSentSigl sentSigl)
                    (mkRDFproperty SentenceForm)
...
--                    then  [mkTripleText (unTokenSigl tokensigl)
--                            (mkRDFproperty NERorig) (tNERorig $ tok) ]
                             -- use what nlp produced
--                    else []
----------------------------------

mkDependenceTypeTriples2 :: LanguageCode -> SentSigl
        ->    DependenceType0 -> [Triple]
mkDependenceTypeTriples2 lang sentSigl  d   =
--              t1 : t2 :
                     ts
    where
        ts = concat $ zipWith (mkDependenceTriple2 lang sentSigl  )
                        ( dtd d)  [1..]
